...
class Zombie:
    ...
    def get_bb(self):
        # fill here
        if self.hp == 2: return self.x - 60, self.y - 100, self.x + 70, self.y + 100
        if self.hp == 1: return self.x - 30, self.y - 50, self.x + 35, self.y + 50
        pass

    def handle_collision(self, group, other):
        if group == 'ball:zombie' and self.hp == 2:
            self.hp-= 1
            self.size_x , self.size_y = 100,100
            self.y -=50
        elif group == 'ball:zombie' and self.hp == 1:
